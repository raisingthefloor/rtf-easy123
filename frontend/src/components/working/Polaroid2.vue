...
<template>
  <div :id="mail.messageId" class='polaroid' :r='mail.r' :t='mail.t' :in='mail.in' @mousedown="thisMousedown($event)" @mouseup="thisMouseup($event)">
    ...
    <div id='frontcontents' style='display:none;'>
      ...
      <div style='position:absolute; left: 120px; top: 128px; font:Times New Roman; font-size:20px;text-overflow: ellipsis;width: 221px;height: 71px;overflow: hidden;'> {{ strip_html_tags(mail.to) }} <span v-if="mail.to.split(',').length > 3" style="position: absolute; top: 45px; right: 5px;">...</span></div>
