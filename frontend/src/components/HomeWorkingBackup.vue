<template>
  <div>
    <div id="containment-wrapper" >
      ...
      <Polaroid></Polaroid>
      <div class='polaroid' @mousedown="polaroidMousedown($event)" @mouseup="polaroidMouseup($event)" r='unread' t='none' in='c'>
        ...
        <div class='envcontents' style='display:none; width:355px'>
          ...
          <div id='letter' style='display:none; top:132px;left:25px; position:absolute; '>
            ...
            <div id='message' style='position:absolute; width:300px; padding: 35px;  font:Times New Roman; font-size:18px; '>
              ...
              <button class='reply' @click="replyClick($event)" style=' padding-left:10px; position:absolute; top:580px; left:25px; '>
                <span style=' font-size:19px;'>Reply</span>
              </button>

              <button class='close' @click="closeClick($event)" style=' padding-left:10px;position:absolute; top:580px; left:120px; padding-right:10px; '>
                <span style=' font-size:19px;'>Keep</span>
              </button>

              <button class='throwaway' @click="throwawayClick($event)" style=' padding-left:10px;  padding-right:10px; position:absolute; top:580px; left: 315px; position: absolute '>
...
export default {
  ...
  methods: {
    initMount() {
      ...
      $("#mail").css('top','0px');
      $("#palbum").css('left','0px');
      $("#palbum").css('top',this.getInt($('#mail').css('margin'))+this.getInt($('#mail').css('height'))+'px');

      $("#icontact").css('left','0px');
      $("#icontact").css('top',2*this.getInt($('#palbum').css('margin'))+2*this.getInt($('#palbum').css('height'))+'px');
      ...
      self.trash_x = self.trash_x-this.buttonwidth-this.MARGIN;
    ...
    flapClick(e) {
      ...
      if(this.isPerson == 1) {
        this.isPerson = 0
        this.$refs.person_contact.style.display = "none"

        $('#person').animate({'width':'0px'}, 1000,function(){
          $(this).hide();
          self.$refs.person.style.display = "none"
        });
      }

      let index = parseInt(e.currentTarget.getAttribute('people'));
      let stop = this.people[index].length;
      for (var p=0;p<=5;p++) {
        if(p < stop) {
          let name = this.people[index][p].split(' ');
          //this.$refs['people_ref_'+p+'_contact_pic']
...
//                $(this).draggable(false);
            ...
            else if(type=="tray")
            {
              ...
              self.trayEmails[this.trayptr]=$(this);
            }
          ...
          $(polar).animate(csstrash1,1000,function() {
            ...
            $(polar).animate(csstrash1,0,function(){
              ...
              $(polar).animate(csstrash1,0,function() {
                ...
                console.log("Deleted email", $(polar))
                self.openedDeleted=flag123;
    ...
    closeClick(e) {
      ...
      $(p).animate({'left':'25px', 'top':'-90px'},650,function() {
        ...
        $(p).animate({'top':'132px'},300,function() {
          $(p).parent().parent().find('.envcontents').rotate3Di('-180',300,{
            sideChange:mySideChange1,
            complete:self.myComplete2(this)
          });
        });
      });
    },
    ...
    trayMousedown() {
      ...
      {
        var deletedEmail1 = self.trayEmails[self.trayptr];
        console.log("deletedEmail1", deletedEmail1)
    ...
    polaroidMouseup(e) {
      ...
      if($(e.currentTarget).find('.envcontents').find('#envelope').attr('src')!='mail/trash_ball.png'&&$(e.currentTarget).find('.envcontents').find('#envelope').attr('src')!='mail/tray_envelope1.png'&&$(e.currentTarget).find('.envcontents').find('#envelope').attr('src')!='mail/tray_envelope2.png')
      {
        ...
        if(b+275>self.trash_x && b1>self.trash_y-64-240-20 && $(e.currentTarget).find('.envcontents').find('#envelope').attr('src')=='mail/envelope.png')
        {
          ...
          $(e.currentTarget).css('z-index',z+2);
          ...
          $(e.currentTarget).css('left',trash_x+34+'px');
          csstrash1.top = self.trash_y+128-73+'px';
          csstrash1.left = self.trash_x+34+'px';
