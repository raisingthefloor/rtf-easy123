      ...
      <div>
        ...
        >
          ...
          <div v-for="item in website_and_folders" :key="item.id" class="files_and_folders_div me-2 mt-3 item" v-bind:class="{ folder_background: (item.type == 'folder') }">
            <span class="files_and_folder_delete badge bg-info mt-1 me-1" @click="editFolder(item.id)" style="right: 30px;" v-if="item.type == 'folder'">
              <font-awesome-icon :icon="['fas','edit']"></font-awesome-icon>
            </span>
            <span class="files_and_folder_delete badge bg-info mt-1 me-1" @click="editWebsite(item.id)" style="right: 60px;" v-if="item.type == 'website'">
              <font-awesome-icon :icon="['fas','edit']"></font-awesome-icon>
            </span>
            <span class="files_and_folder_delete badge bg-danger mt-1 me-1" @click="deleteFileAndFolder(item.id)">
              <font-awesome-icon :icon="['fas','trash']"></font-awesome-icon>
            </span>
            <span v-if="item.type == 'website'" class="files_and_folder_fav badge text-dark mt-1 me-1" v-bind:class="{ 'bg-light': !item.fav, 'bg-warning': item.fav }" @click="favFileAndFolder(item.id)">
        ...
        <div class="col-md-6">
          ...
          <form action="" @submit.prevent="updateWebsite()">
            ...
            <div class="mb-3">
              <label for="image_for_button" class="form-label">Image for button</label>
              <div class="row">
                <div v-if="edit_website_object && change_edit_image == false">
                  <img :src="edit_website_object.image" alt="" v-if="edit_website_object.imageType == 'favcon'">
                  <easy-web-image :image="edit_website_object" v-if="edit_website_object.imageType == 'uploaded'"></easy-web-image>
                  <button type="button" class="btn btn-sm btn-warning mt-2" @click="changeEditImage()">Change Image</button>
                </div>
                <div class="col-md-6" v-if="change_edit_image">
                  <div v-if="website_autogenerated_image">
                    <img src="@/assets/images/no-image.png" alt="" id="image_for_button" style="max-width: 200px; max-height: 152px;">
                  </div>
                  <div v-if="website_autogenerated_favicon">
                ...
                <div class="col-md-6" v-if="change_edit_image">
                  <a href="javascript:void(0)" class="btn btn-sm btn-outline-primary d-block mb-2" @click="useAutoGeneratedFavicon()">Use Favicon</a>
                  <a href="javascript:void(0)" class="btn btn-sm btn-outline-primary d-block mb-2" @click="useUploadedImage()">Upload Image</a>
                  <a href="javascript:void(0)" class="btn btn-sm btn-warning d-block mb-2" @click="usePrevEditImage()">Use Previous Image</a>
...
export default {
  ...
  methods: {
    ...
    /** edit folder **/
    editFolder(item_id) {
      console.log("Edit folder", item_id)
    },
    ...
    editWebsite(item_id) {
      ...
      //get editing item
      let website = this.website_and_folders.find(obj => obj.id == item_id)
      //console.log(website)
    ...
    updateWebsite() {
      ...
      else if (this.website_uploaded_image && (!this.websiteImageFileName || this.websiteImageFileName == "") && this.change_edit_image)
      {
        this.website_image_error = "Image is not valid"
      }
      else if (this.website_autogenerated_favicon && (!this.website_sample_image_url || this.website_sample_image_url == "") && this.change_edit_image)
      {
        this.website_image_error = "Image is not valid"
      }
      else
      {
        ...
        //validation success
        /*axios.post(process.env.VUE_APP_API_HOST_NAME+"/api/assistant/user/easyweb/update-website", {
          id: self.$route.params.id,
          text_to_put_on_button: self.text_to_put_on_button,
          website_url: self.website_url,
          website_image: self.website_image,
    ...
    openFolderOrWebsite(item) {
      console.log("openFolderOrWebsite")
      if(item.type == "folder")
      {
        //open folder
