      ...
      else
      {
        ...
        .then((response) => {
          ...
          else if(response.data.message == "website_loading_failed")
          {
            ...
            swal({
              title: "Failed",
              text: "This website does not allow itself to be open within another application like this. Do you want to have it open in a new browser tab?",
              icon: "error",
              buttons: ["No, don’t add this website button", {
                text: "Yes, open in a new tab",
                closeModal: false,
              }]
            })
            .then((valueFailed) => {
