        ...
        >
          <div class="files_and_folders_div me-2 mt-3 folder_background" slot="header" @click="openMyFavoritesFolder()">
            <div class="text-center">
              <h6 class="text-center mt-3"><b>{{ $t('assistant_module.my_favorites') }}</b></h6>
...
<!--                  <img :src="getImageURL(item)" alt="" v-if="item.type != 'folder'" class="website_image" style="">-->
              ...
              <div class="add-button" @click="addSiteInFolder()" style="margin-top: 70px; display: inline-block;" v-if="!show_favorites_folder">
                ...
                <div>
                  {{ $t('assistant_module.add_new_site') }}
                </div>
              </div>
            </draggable>

            <button class="btn btn-sm btn-secondary" style="position:absolute; top: 17px; right: 4px;" @click="closeFolder()">
              {{ $t('close') }}</button>
          </div>
    ...
    <div v-if="add_site_or_folder_form">
      <div class="text-center mt-5">
        <h4 style="display:inline-block;">{{ $t('assistant_module.add_new') }}</h4>
        <a href="javascript:void(0)" style="display:inline-block;" @click="closeSiteOrFolder()" class="ms-5"><span class="badge bg-secondary">{{ $t('close') }}</span></a>
        <div class="mt-3">
          <button class="btn btn-primary btn-lg me-2" @click="addWebsiteForm()"><i class="fas fa-globe"></i> {{ $t('assistant_module.website') }}</button>
          <button class="btn btn-primary btn-lg" @click="addFolderForm()"><i class="fas fa-folder"></i> {{ $t('assistant_module.folder') }}</button>
        </div>
      </div>
    ...
    <div v-if="add_website_form">
      <div class="row">
        ...
        <div class="col-md-6">
          <h4 class="mt-3">{{ $t('assistant_module.add_website') }} <span v-if="selected_folder.name">{{ $t('assistant_module.to_folder') }} {{ selected_folder.name }}</span></h4>
          <form action="" @submit.prevent="addWebsite()">
            <div class="mb-3">
              <label for="text_to_put_on_button" class="form-label">{{ $t('assistant_module.text_to_put_on_button') }}</label>
            ...
            <div class="mb-3">
              <label for="website_url" class="form-label">{{ $t('assistant_module.website_url') }}</label>
            ...
            <div class="mb-3">
              <label for="image_for_button" class="form-label">{{ $t('assistant_module.image_for_button') }}</label>
              <div class="row">
                <div class="col-md-6">
                  ...
                  <div v-if="website_autogenerated_favicon">
                    ...
                    <span v-if="favicon_loading">{{ $t('loading_progress') }}</span>
                  </div>
                ...
                <div class="col-md-6">
                  <a href="javascript:void(0)" class="btn btn-sm btn-outline-primary d-block mb-2" @click="useAutoGeneratedFavicon()">{{ $t('assistant_module.use_favicon') }}</a>
                  <a href="javascript:void(0)" class="btn btn-sm btn-outline-primary d-block mb-2" @click="useUploadedImage()">{{ $t('assistant_module.upload_image') }}</a>
                </div>
              </div>
            </div>
            <div class="mb-3">
              <button class="btn btn-primary me-2" type="submit">{{ $t('save') }}</button>
              <button class="btn btn-secondary" @click="closeAddWebsiteForm">{{ $t('close') }}</button>
            </div>
    ...
    <div v-if="edit_website_form">
      <div class="row">
        <div class="col-md-3"></div>
        <div class="col-md-6">
          <h4 class="mt-3">{{ $t('assistant_module.edit_website') }} <span v-if="selected_folder.name">{{ $t('assistant_module.to_folder') }} {{ selected_folder.name }}</span></h4>
          <form action="" @submit.prevent="updateWebsite()">
            ...
            <div class="mb-3">
              <button class="btn btn-primary me-2" type="submit">{{ $t('save') }}</button>
              <button class="btn btn-secondary" @click="closeAddWebsiteForm" type="button">{{ $t('close') }}</button>
            </div>
        ...
        <div style="width: 75vw; height: 75vh;background-color: #ceb874;margin: auto; overflow-y: auto; position: relative;" class="p-3">
          <form @submit.prevent="saveFolder()">
            <div class="row g-3 align-items-center">
              <div class="col-auto">
                <label for="folder_name" class="col-form-label">{{ $t('assistant_module.folder_name') }}</label>
              </div>
              <div class="col">
                <input type="text" id="folder_name" v-model="folder_name" class="form-control" autocomplete="off">
              </div>
              <div class="col-auto">
                <button class="btn btn-primary" type="submit">{{ $t('assistant_module.add') }}</button>
                <button class="btn btn-secondary ms-3" @click="closeAddFolderForm">{{ $t('close') }}</button>
              </div>
            </div>
          </form>
        </div>
...
export default {
  ...
  watch: {
    website_url(newVal) {
      if(this.edit_website)
      {
        if(this.edit_website.link == newVal)
        {
          this.openInNewTab = this.edit_website.openInNewTab
        }
        else
        {
  ...
  methods: {
    ...
    async deleteFileAndFolder(id, parent_id) {
      ...
      let willDelete = await swal({
        title: this.$t('assistant_module.add_you_sure_you_want_to_delete', {website: website.name}),
        text: this.$t('assistant_module.you_can_add_it_anytime'),
        icon: "warning",
        buttons: [this.$t('assistant_module.dont_delete'), this.$t('assistant_module.yes_i_am_sure')],
        dangerMode: true,
      })
    ...
    addWebsite() {
      ...
      {
        this.text_to_put_on_button_error = this.$t('assistant_module.please_enter_correct_value')
      }
      else if (!this.website_url || this.website_url == "")
      {
        this.website_url_error = this.$t('assistant_module.please_enter_correct_website_url')
      }
      else if (this.website_uploaded_image && (!this.websiteImageFileName || this.websiteImageFileName == ""))
      {
        this.website_image_error = this.$t('assistant_module.image_is_not_valid')
      }
      ...
      else
      {
        ...
        .then((response) => {
          ...
          else if(response.data.message == "website_loading_failed")
          {
            ...
            swal({
              title: self.$t('failed'),
              text: self.$t('assistant_module.swal_website_not_open_in_iframe_warning'),
              icon: "error",
              buttons: [self.$t('assistant_module.dont_add_website_button'), {
                text: self.$t('assistant_module.yes_open_in_new_tab'),
                closeModal: false,
              }]
            })
            .then((valueFailed) => {
    ...
    updateWebsite() {
      ...
      if(!this.text_to_put_on_button || this.text_to_put_on_button == "")
      {
        this.text_to_put_on_button_error = self.$t('assistant_module.please_enter_correct_value')
      }
      else if (!this.website_url || this.website_url == "")
      {
        this.website_url_error = self.$t('assistant_module.please_enter_correct_website_url')
      }
      ...
      {
        this.website_image_error = self.$t('assistant_module.image_is_not_valid')
      }
