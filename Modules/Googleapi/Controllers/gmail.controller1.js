...
class GmailController {
    ...
    listUnreadMessages(auth) {
        ...
        gmail.users.messages.list({
            ...
            if (err) return console.log('The API returned an error: ' + err);
            //res.send([res.data]);
            //console.log("we are returning data")
            //return res.data
            //console.log("messages:: ", res.data)

            //auth.expressResponse.send(res.data)
            ...
            let counter = 1
            //formatedResponse.push(allMails)
            if(allMails.length)
                allMails.forEach((element, index) => {
                    ...
                    gmail.users.messages.get({
                        ...
                        if (err) return console.log('The API returned an error: ' + err);
                        //console.log("counter", counter++, res.data)
                        let mail_data = res.data
                        /*if(mail_data.payload.mimeType == "multipart/alternative")
                        {
                            //normal data
                            if(mail_data.payload.parts)
                            {
                                ...
                                //let buff = new Buffer.from(mail_data.payload.parts[1].body.data, 'base64');
                        ...
                        else if (mail_data.payload.mimeType == "multipart/mixed")
                        {
                            //normal data with attachment
                            //get alternative part

                            let obj_alternative_mime = mail_data.payload.parts.find(x => x.mimeType === 'multipart/alternative')
                            console.log("alternative data", obj_alternative_mime)
                            if(obj_alternative_mime.payload.parts)
                            {
                                let obj = obj_alternative_mime.payload.parts.find(x => x.mimeType === 'text/html')
                                let buff = new Buffer.from(obj.body.data, 'base64');
                                //let buff = new Buffer.from(mail_data.payload.parts[1].body.data, 'base64');
                            ...
                            console.log("obj_alternative_mime", obj_alternative_mime)
                        }*/
                        //console.log("mail_data.payload", mail_data.payload)
                        //console.log("mail_data.payload.body.data", mail_data.payload.body.data)
                        ...
                        {
                            //console.log("Yes, you are here", formatedResponse)
                            //write in file & store data
                            /*fs.writeFile(__dirname+'/data/unreadmail.txt', JSON.stringify(formatedResponse), (err) => {
                                if (err) {
                                    return console.error(err);
                                }
                                else {
                                    return console.log('response written:::', "here", new Date())
                                }
